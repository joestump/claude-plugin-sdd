...
- MUST default `qmd embed` to **background** on a CPU-only machine without prompting (per ADR-0026's embed policy — the pre-v5 three-way `AskUserQuestion` prompt was removed as friction); explicit `--foreground` / `--skip` flags in `$ARGUMENTS` override the default. Embedding the EmbeddingGemma 300M model on CPU runs at roughly 1s per chunk, so a foreground embed silently blocks the session for many minutes
