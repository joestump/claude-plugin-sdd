...
How the SDD plugin is structured internally, and the conventions it follows.
...
│   └── claude-md-template.md # CLAUDE.md template for /sdd:init
...
└── docs/                     # This plugin's own design artifacts
    ├── prds/                 # Product Requirements Documents (optional)
...
1. **YAML frontmatter** — declares metadata that Claude uses to decide when and how to invoke the skill:
   - `name`: The skill identifier (matches the slash command, e.g., `plan` for `/sdd:plan`)
...
This is why the `/sdd:docs` skill splits its three modes (scaffold, integration, upgrade) into separate reference files rather than inlining them. When a user runs `/sdd:docs` in scaffold mode, only `references/scaffold-mode.md` gets loaded — the integration and upgrade instructions stay out of context entirely.
...
- **Tool discovery** (`plan`, `organize`, `enrich`, `work`, `review`): Include `ToolSearch` to dynamically discover tracker-specific MCP tools (GitLab, Jira, Linear) at runtime. GitHub and Gitea go through the `gh` and `tea` CLIs instead.
...
The file is first created when `/sdd:plan` detects and saves a tracker preference. It is subsequently read by `plan`, `organize`, `enrich`, `work`, and `review`.
...
The template in `references/claude-md-template.md` is what `/sdd:init` injects into a project's `CLAUDE.md` file. It gives Claude architecture awareness in every session by documenting:

- Where PRDs, ADRs, and specs live
- The full skill table with descriptions
- The governing comment convention
- The recommended workflow (Capture Intent, Decide, Specify, Plan, Enrich, Build, Validate)
...
This manifest is created by `/sdd:docs` in the user's project root. It tracks which files the docs skill has generated, along with their checksums. This enables the upgrade mode to detect which generated files have been manually modified by the user and which can be safely overwritten during a docs regeneration.
...
- **`templates/integration/`**: Used by integration mode. Contains a Docusaurus build-time plugin (`sync-spec-docs`) that hooks into an existing Docusaurus installation. It uses `loadContent()` and `getPathsToWatch()` for hot module reloading. Output goes to `{site}/docs/architecture/` (gitignored), with components placed in `{site}/src/components/design-docs/`.

## Design Artifacts

The plugin dogfoods its own architecture governance. The `docs/adrs/` and `docs/openspec/specs/` directories contain the ADRs and specifications that govern the plugin's own development. At the time of writing, the plugin has 36 ADRs and 25 specs — and no PRDs, which is the optionality rule applied to the plugin itself: its own work is engineering, with no client signing off on it.

For example, ADR-0011 and SPEC-0010 govern how `/sdd:plan` decomposes specs into issues, and ADR-0013 and SPEC-0012 define the `--scrum` ceremony mode. Changes to the plugin go through the same Decide-Specify-Build cycle that users follow in their own projects.

## Conventions

- **Skill naming**: kebab-case directory name matches the slash command (e.g., `skills/plan/SKILL.md` maps to `/sdd:plan`)
- **PRD numbering**: `PRD-XXXX` (zero-padded, sequential) — optional artifact, client-facing capabilities only
...
- **Governing comments**: Code leaves `// Governing: ADR-XXXX, SPEC-XXXX REQ "Name"` traces so future sessions and `/sdd:check` can trace implementation back to decisions
- **Team review**: `--review` flag spawns a drafter/auditor + reviewer pair with a maximum of 2 revision rounds
- **Scrum ceremonies**: `--scrum` flag on `/sdd:plan` and `/sdd:audit` spawns 5-agent teams for full grooming and triage ceremonies
