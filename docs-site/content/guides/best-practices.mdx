...
Tips for getting the most out of the SDD plugin.

## Start Every Session with `/sdd:prime`
...
`/sdd:prime` loads your ADRs and specs into context so Claude works *with* your architecture instead of against it. If you're focusing on a specific area, use topic filtering (`/sdd:prime authentication`) to load only the relevant artifacts and keep the context window focused.

Make it a habit. Prime before you ask anything else.

## Write a PRD Only When Someone Is Signing Off

`/sdd:prd` exists for client-facing capabilities — work where a person outside the codebase has to read what you intend to build and agree to it. It captures the request in their language, with a blast radius confirmed against the code and success criteria in EARS syntax that an agent can actually check.

The rule that keeps it useful is the one about *not* using it. Most decisions are pure engineering and have no product intent upstream, so the plugin never reports a missing PRD — no skill flags an ADR or spec for lacking one. Writing PRDs for internal work produces ceremony that nobody signs and nobody reads, and it dilutes the signal for the documents that are genuinely client-facing.

If you cannot name the person who will approve it, you do not need one.
...
Use `/sdd:adr` to capture decisions while the reasoning is fresh. An ADR written in five minutes today saves hours of archaeology later. The "Considered Alternatives" section is the most valuable part — it prevents future you from re-litigating settled questions.
...
Small decisions get a small record. `/sdd:adr --quick` writes a lightweight ADR — context, decision, a one-line alternative, consequences — with no options matrix and no diagram. Reach for it whenever the honest alternative is "don't do this." A padded Considered Options list is worse than a short one: it makes the record look weighed when it was not, and it costs every future reader the time to discover the extra options were never real. The moment a second genuinely viable option turns up, that's a full ADR.
...
The `/sdd:spec` skill enforces this pairing when you use it. But manual edits — quick fixes in an editor, tweaks during code review — can cause the two files to diverge silently. Run `/sdd:check` after any manual edits to catch misalignment early, before it propagates into implementation.
...
Because `/sdd:plan` does more than create a task list. It derives issues with acceptance criteria directly from spec scenarios — each issue gets clear, verifiable goals that a worker can implement and validate independently. Skip planning and workers have to interpret the spec themselves, which leads to inconsistent implementations, missed requirements, and rework.

Planning also establishes dependency ordering. Some issues block others. `/sdd:plan` captures these relationships so `/sdd:work` can pick up issues in the right sequence instead of starting with something that depends on unfinished work.
...
The `--scrum` flag on `/sdd:plan` runs the full ceremony: spec audit, decomposition, five-agent grooming, organize, enrich, and a sprint report. Engineer B's high-bar review is the key differentiator — it challenges vague acceptance criteria, flags missing edge cases, and pushes back on over-scoped issues before they become implementation problems.

Similarly, `--scrum` on `/sdd:audit` runs a triage ceremony that validates your source of truth, groups findings into themes, and runs a five-agent triage where the Architect makes final calls on whether to fix the code or update the artifact.
...
## Let `/sdd:work` Propose from the Backlog

Running `/sdd:work` without arguments doesn't just pick a random issue. It analyzes the full backlog — considering dependency ordering, blocked vs. unblocked issues, and unblocking impact — then proposes a batch of issues to work on.
...
`/sdd:check` is cheap — run it often during implementation, like a linter. It does a quick-pass comparison of your code against governing ADRs and specs, flagging obvious divergences. Get in the habit of running it before committing, especially after implementing a tricky requirement.

`/sdd:audit` is comprehensive — run it at milestones. It does a deep alignment analysis across all artifacts and implementation, catching subtle drift that quick checks miss. Use it before releases, after large merges, or whenever you suspect things have gotten out of sync.
...
| I want to... | Use |
|---|---|
| Capture what a client asked for | `/sdd:prd` |
| Document a decision | `/sdd:adr` |
| Formalize requirements | `/sdd:spec` |
| Break a spec into issues | `/sdd:plan` |
| Group issues into projects retroactively | `/sdd:organize` |
| Add branch names to existing issues | `/sdd:enrich` |
| Implement planned issues | `/sdd:work` |
| Review and merge PRs | `/sdd:review` |
| Quick alignment check | `/sdd:check` |
| Full project audit | `/sdd:audit` |
| Find undocumented decisions in code | `/sdd:discover` |
| Generate a docs site | `/sdd:docs` |
| Set up a new project | `/sdd:init` |
| Load context at session start | `/sdd:prime` |
| Review and respond to PR feedback | `/sdd:respond` |
| Index artifacts for semantic search | `/sdd:index` |
| See what exists | `/sdd:list` |
| Update artifact status | `/sdd:status` |
...
```
/sdd:init
/sdd:prd [client-facing capability]   # only if someone is signing off
/sdd:adr [first major decision] --review
/sdd:spec [first capability] --review
...
/sdd:adr [discovered decision]
/sdd:spec [discovered capability]
/sdd:plan SPEC-0001
```

Don't try to document everything at once. `/sdd:discover` will surface implicit architecture — conventions, patterns, and decisions buried in the code. Pick the most consequential ones to formalize first. You can always come back for more.
...
/sdd:plan SPEC-0003     # re-plan updated spec
```
