...
The SDD plugin offers two modes for turning specs into actionable work: a
...
After writing a spec, break it into trackable issues with `/sdd:plan`:
...
1. **Detects your tracker** — checks for Beads, GitHub, GitLab, Gitea, Jira, and Linear.
   Saves your choice to `.claude-plugin-sdd.json` so you're never asked again.
...
- You haven't run `/sdd:audit` recently and want freshness validation
...
- If existing tracker issues reference work that has no backing spec, the planner
  produces **spec proposals** — ready-to-use `/sdd:spec` commands for each
  unspec'd area.
...
No need to run `/sdd:organize` or `/sdd:enrich` separately.
...
### Spec Proposals 📋
- Unspec'd: Rate Limiting — run `/sdd:spec Rate Limiting` to formalize
```

## Retroactive Enrichment

Already have issues from a previous `/sdd:plan` run? Use these to catch up
without re-creating issues:

```
/sdd:organize SPEC-0003     # group into projects
/sdd:enrich SPEC-0003       # add branch names and PR conventions
```
...
/sdd:adr Add WebSocket-based notifications --review

# 2. Write the spec
/sdd:spec Notification Service

# 3. Plan with team grooming
/sdd:plan SPEC-0004 --scrum
