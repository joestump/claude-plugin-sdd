...
# ADR-0015: Markdown-Native Configuration — Eliminate `.claude-plugin-sdd.json`

## Context and Problem Statement

The SDD plugin currently stores runtime configuration in `.claude-plugin-sdd.json` — tracker choice, branch conventions, PR conventions, worktree settings, and review settings. This creates a split source of truth with `CLAUDE.md`, which already carries architecture context and instructions that Claude Code interprets natively. How should the plugin consolidate configuration into a single, conflict-resistant, human-readable format that leverages Claude Code's existing capabilities?

Three production projects (spotter, joe-links, claude-ops) exposed two concrete failures of JSON config:

1. **Split truth**: Developers had to keep `.claude-plugin-sdd.json` and `CLAUDE.md` synchronized. When they diverged, agent behavior became unpredictable — one source said "use GitHub Issues" while the other referenced Gitea.
2. **Merge conflict hotspot**: In claude-ops, `.claude-plugin-sdd.json` was modified by every single parallel PR in a sprint. It was the #1 merge conflict source across all three repos because every agent that ran `/sdd:work` or `/sdd:plan` updated shared fields in the same JSON file.
...
* **Option 1**: Keep `.claude-plugin-sdd.json` with JSON Schema validation
...
* Bad, because migration is required for existing users who have `.claude-plugin-sdd.json` in their repos (mitigated by a one-time migration in `/sdd:init` that reads the JSON and writes equivalent CLAUDE.md sections)
...
1. All skills read configuration from `CLAUDE.md` sections instead of `.claude-plugin-sdd.json`
2. `/sdd:init` writes configuration as structured markdown sections in `CLAUDE.md` instead of generating a JSON file
3. `/sdd:init` detects existing `.claude-plugin-sdd.json` files and offers to migrate them to `CLAUDE.md` sections
4. Workspace projects with per-module `CLAUDE.md` files correctly resolve module-specific configuration
5. No skill references `.claude-plugin-sdd.json` for reading or writing configuration
...
Replace `.claude-plugin-sdd.json` with `.claude-plugin-sdd.yml` for more human-readable structured config.
...
Replace `.claude-plugin-sdd.json` with `.claude-plugin-sdd.toml`.
...
- Migration from `.claude-plugin-sdd.json` will be handled by `/sdd:init`, which will detect the JSON file, read its contents, write equivalent `CLAUDE.md` sections, and offer to delete the JSON file.
...
- Evidence base: `.claude-plugin-sdd.json` was the worst file hotspot in claude-ops (all 7 parallel PRs modified it). In the parallel agent coordination analysis across three repos, it was the only file that was guaranteed to conflict in every sprint.
