...
* **Amendment (2026-09-03):** a lightweight tier was added for decisions with a single viable option — `/sdd:adr --quick`, marked `tier: quick` in frontmatter. It keeps Context, Decision, a one-line Alternative, and Consequences, and omits Decision Drivers, the options matrix, and the Architecture Diagram. The mandatory-diagram and two-options rules above now apply to full-tier ADRs only; see SPEC-0003 REQ "MADR ADR Format" and REQ "Architecture Diagrams". Motivation: the no-threshold guidance in the docs ("don't wait for big enough") combined with a fixed template produced padded strawman option lists in practice.
