...
2. **Cycle detection**: The graph MUST be a DAG except for symmetric `related` edges. Cycle detection MUST be run over a **direction-normalized** graph: `enables` and `governs` point downstream (source unblocks / governs its target) while `supersedes`, `extends`, `implements`, and `requires` point upstream (source builds on / follows its target), so downstream-pointing edges MUST be inverted before the depth-first search. A cycle in any edge type (`supersedes`, `extends`, `enables`, `governs`, `implements`, `requires`) after normalization MUST be reported as a hard error including the full cycle path. A semantically-consistent mixed pair across directions (e.g., `ADR-A enables: [ADR-B]` with `ADR-B extends: [ADR-A]`) MUST NOT be reported as a cycle.
