...
Cross-cutting security defaults, code quality checks, frontend quality standards, and governing comment reform integrated into the existing design plugin skill pipeline. Rather than introducing standalone security or quality skills, these guardrails are woven into `/design:spec`, `/design:plan`, `/design:check`, and `/design:init` so that security and quality are part of the normal workflow at every stage — specification authoring, sprint planning, and code verification.

This spec formalizes requirements from ADR-0018 (Security-by-Default for Web Specifications), ADR-0019 (Frontend Quality Standards), and ADR-0020 (Governing Comment Reform). Evidence is drawn from a systematic review of three production projects (spotter, joe-links, claude-ops) built with the design plugin.

## Requirements

### Requirement: Mandatory Security Section in Web Specs

When `/design:spec` creates or updates a specification that involves HTTP endpoints, web server routes, API definitions, or browser-facing UI, the skill MUST inject a "Security Requirements" section into the spec. The section MUST cover all of the following topics:
...
- **WHEN** a user runs `/design:spec` for a capability involving HTTP endpoints (e.g., a REST API, a web dashboard, an HTMX application)
...
- **WHEN** a user runs `/design:spec` for a capability that does not involve HTTP endpoints (e.g., a CLI tool, a data migration script, an internal Go library)
...
When `/design:plan` creates issues for stories that involve HTTP endpoints, the skill MUST include a security checklist in the issue body. The checklist MUST cover:
...
- **WHEN** `/design:plan` creates an issue for a story that implements or modifies an HTTP endpoint
...
- **WHEN** `/design:plan` creates an issue for a story that does not involve HTTP endpoints (e.g., a database migration, a background job, a library refactor)
...
`/design:check` MUST scan source code for the following dangerous patterns and flag each as a security finding:
...
- **WHEN** `/design:check` scans a Go file containing `io.ReadAll(r.Body)` and the body is not wrapped in `http.MaxBytesReader` within the same function or a calling function in the same file
...
- **WHEN** `/design:check` scans JavaScript files or HTML templates containing `.innerHTML =` or `.innerHTML +=` assignments
...
- **WHEN** `/design:check` scans HTML files containing `<script src="https://...">` or `<link href="https://..."` without an `integrity` attribute
...
- **WHEN** `/design:check` scans route registration code (e.g., `http.HandleFunc`, `router.GET`, `app.get`) and the handler chain does not reference auth middleware
...
- **THEN** the security lint pattern would have flagged all three during `/design:check`, enabling remediation before production deployment

### Requirement: Auth-by-Default

When `/design:spec` generates specifications for web-facing capabilities, all endpoints MUST require authentication by default. Endpoints that are public (unauthenticated) MUST be explicitly declared in the spec with a justification for each.
...
- **WHEN** `/design:spec` generates an endpoint table for a web API spec
- **THEN** every endpoint in the table defaults to "Auth: Required"

#### Scenario: Explicit public endpoint declaration

- **WHEN** a spec author declares an endpoint as public during `/design:spec`
...
For projects where `/design:spec` or `/design:init` detects a `go.mod` file at the project root, the following Go-specific quality guidelines MUST be applied:

1. `/design:init` SHOULD suggest creating a structured logging ADR that mandates `slog` over `log.Printf` or `fmt.Printf` when initializing a Go project for the first time.

2. `/design:spec` MUST include error handling guidelines in the generated spec when the spec involves Go code:
...
4. `/design:plan` SHOULD create a CI story for Go projects that includes `go vet`, `go test -race`, and `golangci-lint` in the CI pipeline.

#### Scenario: Go project initialization with logging suggestion

- **WHEN** a user runs `/design:init` in a directory containing a `go.mod` file and no existing logging ADR
...
- **WHEN** `/design:spec` generates a spec for a Go project (detected via `go.mod`)
...
- **WHEN** `/design:spec` generates a spec involving goroutines or background workers
...
- **WHEN** `/design:plan` creates stories for a Go project and no CI story exists in the current plan
...
- **THEN** the `/design:init` logging ADR suggestion would have established `slog` as the standard from day one

### Requirement: Accessibility Requirements for UI Specs

When `/design:spec` creates or updates a specification that involves user interface components (HTML templates, browser-rendered pages, interactive UI elements), the skill MUST inject an "Accessibility Requirements" section. The section MUST require:
...
- **WHEN** a user runs `/design:spec` for a capability involving HTML templates, browser UI, or interactive frontend components
...
- **WHEN** a user runs `/design:spec` for a capability with no UI component (e.g., a REST API consumed by other services, a message queue consumer)
...
When `/design:plan` creates stories for a spec that touches UI components (HTML templates, JavaScript, CSS, HTMX interactions), the skill MUST create companion test stories alongside the feature stories. Companion test stories MUST cover:
...
- **WHEN** `/design:plan` creates stories for a spec that includes HTML templates and HTMX interactions
...
- **WHEN** `/design:plan` creates stories for a spec that involves only backend logic with no UI components
...
`/design:check` MUST scan HTML templates, JavaScript files, and related frontend assets for the following template quality patterns:
...
- **WHEN** `/design:check` scans HTML templates and finds identical `<script>` blocks in the same file or across multiple template files
...
- **WHEN** `/design:check` scans HTML templates and finds a `<script>` tag referencing `cdn.tailwindcss.com`
...
- **WHEN** `/design:check` scans a project and finds references to more than one JS interaction framework (e.g., both HTMX and Alpine.js CDN URLs or imports)
...
- **WHEN** `/design:check` scans HTML templates and finds navigation or sidebar markup (`<nav>`, elements with `role="navigation"`) rendered in more than one template file without using a shared partial/include
...
- **THEN** the template quality detection would have flagged both patterns during `/design:check`

### Requirement: Governing Comment Format

All design plugin skills that generate or suggest governing comments MUST use file-level governing comment blocks instead of per-line annotations. The canonical format is a single block at the top of the file:
...
Skills MUST NOT create separate PRs, issues, or stories for retroactive governing comment addition. Governing comments MUST be added in the same PR that implements the governed feature. `/design:plan` MUST NOT generate stories whose sole purpose is adding governing comments to existing files.

#### Scenario: File-level governing block in generated code

- **WHEN** `/design:work` generates a new file implementing requirements from SPEC-0003 and decisions from ADR-0001
...
- **WHEN** `/design:plan` generates a sprint plan and an existing file lacks governing comments
