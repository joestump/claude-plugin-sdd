...
Skills MUST read configuration from a `### Design Plugin Configuration` section in `CLAUDE.md` instead of from `.claude-plugin-design.json`. The configuration section MUST support the following subsections, each using markdown lists with bold keys:
...
- **WHEN** a skill needs the tracker type and CLAUDE.md contains a `### Design Plugin Configuration` section with a `#### Tracker` subsection listing `- **Type**: gitea`
- **THEN** the skill uses `gitea` as the tracker type without reading any JSON file

#### Scenario: Missing configuration section

- **WHEN** a skill needs configuration and CLAUDE.md does not contain a `### Design Plugin Configuration` section
- **THEN** the skill falls through to tracker detection (the existing auto-detection flow) and SHOULD suggest running `/design:init` to persist configuration

#### Scenario: Partial configuration

- **WHEN** CLAUDE.md contains a `### Design Plugin Configuration` section but the `#### Branch Conventions` subsection is absent
...
1. Read the `### Design Plugin Configuration` section from the project-root CLAUDE.md
...
- **WHEN** `/design:plan` needs the tracker type and branch conventions
...
`/design:init` MUST detect an existing `.claude-plugin-design.json` file in the project root. When detected, `/design:init` MUST read the JSON contents, translate each key-value pair into the equivalent CLAUDE.md markdown section format, and offer to write the configuration into CLAUDE.md using `AskUserQuestion`.

After successful migration, `/design:init` SHOULD offer to remove the `.claude-plugin-design.json` file. The user MUST confirm deletion; the skill MUST NOT delete the file without explicit consent.

The migration MUST preserve all configuration values exactly. If CLAUDE.md already contains a `### Design Plugin Configuration` section, the migration MUST merge new values into existing sections rather than duplicating or overwriting.

#### Scenario: Fresh migration from JSON

- **WHEN** the user runs `/design:init` and `.claude-plugin-design.json` exists with tracker, branch, and PR settings, and CLAUDE.md has no configuration section
- **THEN** `/design:init` writes a `### Design Plugin Configuration` section to CLAUDE.md with all settings from the JSON file and offers to delete the JSON file

#### Scenario: Migration with existing CLAUDE.md config

- **WHEN** `.claude-plugin-design.json` exists and CLAUDE.md already has a `### Design Plugin Configuration` section with partial settings
- **THEN** `/design:init` merges the JSON values into the existing CLAUDE.md section, preserving values already present in CLAUDE.md (CLAUDE.md takes precedence on conflicts)

#### Scenario: User declines JSON deletion

- **WHEN** `/design:init` offers to delete `.claude-plugin-design.json` after migration and the user declines
...
- **WHEN** the user runs `/design:adr --module service-a` in a workspace with modules `service-a` and `service-b`
...
- **WHEN** the user runs `/design:check --module nonexistent` and the workspace has modules `frontend` and `backend`
...
- **WHEN** the user runs `/design:list --module anything` on a project without `.gitmodules` or workspace table
- **THEN** the flag is silently ignored and the skill operates on the project root

### Requirement: Cross-Module Aggregation

`/design:prime`, `/design:audit`, and `/design:docs` MUST present unified views that aggregate artifacts across all discovered modules. Aggregated output MUST include a module label for each artifact so users can identify which module an artifact belongs to.

`/design:check` MUST support per-module targeting via the `--module` flag. When no `--module` flag is provided, `/design:check` MUST check all modules and group findings by module.

`/design:list` MUST aggregate ADRs and specs across all modules with module labels when operating in a workspace.

#### Scenario: Prime loads all modules

- **WHEN** the user runs `/design:prime` in a workspace with modules `api` and `worker`
...
- **WHEN** the user runs `/design:audit` in a workspace
...
`/design:init` MUST detect `.gitmodules` in the project root. When detected, `/design:init` MUST offer workspace setup via `AskUserQuestion`, which includes:
...
4. Writing a `### Design Plugin Configuration` section in the root `CLAUDE.md` with shared configuration (tracker, branch conventions, etc.)

`/design:init` SHOULD detect whether each submodule already has a `CLAUDE.md` and skip those that are already configured, offering to update them only if the user confirms.
...
- **WHEN** the user runs `/design:init` in a project with `.gitmodules` listing `service-a` and `service-b`, and neither submodule has a `CLAUDE.md`
- **THEN** `/design:init` creates `CLAUDE.md` in both `service-a/` and `service-b/` with architecture context sections, and writes a `### Workspace Modules` table and `### Design Plugin Configuration` section in the root `CLAUDE.md`

#### Scenario: Partial workspace init

- **WHEN** `/design:init` detects `.gitmodules` and `service-a/CLAUDE.md` already exists with architecture context
- **THEN** `/design:init` skips `service-a` (unless the user requests an update) and creates `CLAUDE.md` for the remaining modules

#### Scenario: Workspace init with existing JSON config

- **WHEN** `/design:init` detects both `.gitmodules` and `.claude-plugin-design.json`
- **THEN** `/design:init` performs JSON migration first (per the Migration requirement), then proceeds with workspace setup using the migrated configuration
